...
function App() {
  ...
  let microsecondsThreeMonths = 1000 * 60 * 60 * 24 * 3 * 30;
  let threeMonthAgo = new Date().getTime() - microsecondsThreeMonths;
  ...
  return (
    <div className="m-2 ">
      ...
      <button
        className="ml-[370px] p-1 border-2 rounded-lg text-center drop-shadow-xl hover:bg-red-500"
        onClick={deleteHistory}
      >
        <span class="material-symbols-outlined">delete</span>
      </button>
      <div>
        <button onClick={() => setFilteredData(historyItems)}>
          All history
        </button>
        <button onClick={topVistedHistory}>Top 10 visted URL</button>
      </div>
